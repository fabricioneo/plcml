// Heurística para o problema robusto
#include <iostream>
#include <iomanip>
#include <fstream>
#include <vector>
...
class Heuristicas{
  public:
      // Parâmetros das instâncias
        ...
        double alpha1; // Coletar valor de alpha via linha de comando
        double Gamma; // tamanho do budget de incerteza
        double gamma1; // porcentagem de robustez
        double receita; // Coletar valor da receita via linha de comando
...
cout << "hub ativo: " << vetor_um_hub[0]+1 << endl;
cout << "valor da solução com um hub ativo: " << vetor_um_hub[1] << endl;

ht->h[vetor_um_hub[0]]=1;
ht->valor_solucao = vetor_um_hub[1];

//ht->heuristica_construtiva(vetor_um_hub[0], vetor_um_hub[1]);
//cout << "valor da solução com a Heurística Construtiva: " << fixed << setprecision(2) << ht->valor_solucao << endl;
double valor_inicial = ht->valor_solucao;


//arquivo saída dos limitantes
//ofstream arq_saida3("Resultados-limitantes-tempo-novo.txt",std::ofstream::app);
		//if (!arq_saida3) {cerr << "Erro arquivo \n"; exit(0);}

		//arq_saida3<<argv[1]<<"\t"<<ht->alpha<<"\t"<<ht->r[0][0]<<"\n"<<fixed << setprecision(2)<<valor_inicial<<"\t"<<fixed << setprecision(2)<<(double)(clock()- ht->inicio_CPU)/CLOCKS_PER_SEC<<endl;

//srand(time(NULL));

//ht->heuristica_parcialmente_gulosa(vetor_um_hub[0], vetor_um_hub[1]);
//cout << "valor da solução com a Heurística parcialmente gulosa: " << fixed << setprecision(2) << ht->valor_solucao << endl;
//double valor_inicial = ht->valor_solucao;
...
//=========Heurística SmartILS-VND========//
if(valor_inicial > EPSILON){
srand(time(NULL));
ht->SmartILS(2);
}
...
ofstream arq_saida("Resultados-heuristica-robusto.txt",std::ofstream::app);
		if (!arq_saida) {cerr << "Erro arquivo \n"; exit(0);}

        arq_saida<<argv[1]<<"\t"<<ht->alpha<<"\t"<<ht->r[0][0]<<"\t"<<ht->gamma1<<"\t"<<fixed << setprecision(2)<<valor_inicial<<"\t"<<fixed << setprecision(2)<<ht->valor_solucao_star<<"\t"<<fixed << setprecision(2)<<(double)(ht->fim_CPU - ht->inicio_CPU)/CLOCKS_PER_SEC<<endl;


ofstream arq_saida2("Resultados-configuracoes.txt",std::ofstream::app);
		if (!arq_saida2) {cerr << "Erro arquivo \n"; exit(0);}

		arq_saida2<< "\n \n" <<argv[1]<<"\t"<<ht->alpha<<"\t"<<ht->r[0][0]<<"\t"<<ht->gamma1<<"\t"<<fixed << setprecision(2)<<valor_inicial<<"\t"<<fixed << setprecision(2)<<ht->valor_solucao_star<<"\t"<<fixed << setprecision(2)<<(double)(ht->fim_CPU - ht->inicio_CPU)/CLOCKS_PER_SEC<<endl;
...
void Heuristicas::Read_main_arg(int argc, char *argv[]){
    ...
    gamma1 = (argc > 4) ? atof(argv[4]) : 0.3; //valor da porcentagem de robustez via linha de comando
}
...
void  Heuristicas::Read_data(char name[]){
    ...
    for (int i = 0; i <  n; i++){ //Determinar a variação máxima de incerteza na demanda de cada par (i,j)
	  for (int j = 0; j < n; j++){
	    w_chapeu[i][j] = 0.3*w[i][j]; //(30% de w[i][j] - raio)
	  }
	}
	...
	Gamma = gamma1*((n*n)+n+(n*(n-1))); //Tamanho do budget de incerteza (porcentagem de robustez)
...
vector<double> Heuristicas::sol_um_hub(){
    ...
    vector<vector<double > > l_aux(n, vector<double>(n));
    vector<double> vet_sol_um_hub(2);
    vector<vector<double > > w_aux(n, vector<double>(n));
    vector<int> vetw(2);
    vector<int> vets(1);
  ...
  multimap<double,vector<int> >::iterator it;
    ...
    phi_aux = 0.0;
        ...
        //Verificar quais pares compensam ser atendidos e calcular quais terão seus parâmetros incertos

        for(int i = 0; i < n; i++){
            for(int j = 0; j < n; j++){
                l_aux[i][j] = r[i][j]*(w[i][j]-w_chapeu[i][j])-( (c[i][k]+c[k][j])*(w[i][j]-w_chapeu[i][j]) );
                if (l_aux[i][j]>EPSILON){

                    mp.insert(pair<double,vector<int> >(-(r[i][j] - (c[i][k]+c[k][j]) )*w_chapeu[i][j], {0, i, j}));
                }
            }
        }

        mp.insert(pair<double,vector<int> >(-s_chapeu[k], {1, k}));

        int m = 0;

        while(mp.size()>0 && m < floor(Gamma)){

                it=mp.begin();

                if( (it->second)[0] == 0 ){
                    w_aux[ it->second[1] ][ it->second[2] ] = it->first;
                }
                else{
                    s_aux = it->first;
                }
        ...
        if(Gamma-floor(Gamma) > EPSILON){
            if(!mp.empty()){
                it=mp.begin();
                if( (it->second)[0] == 0 ){
                    w_aux[ it->second[1] ][ it->second[2] ] = -(Gamma-floor(Gamma))*it->first;
                }
                else{
                    s_aux = (Gamma-floor(Gamma))*it->first;
                }
            }
        }
        ...
        for(int i = 0; i < n; i++){
            for(int j = 0; j < n; j++){
                l[i][j] = r[i][j]*w[i][j]-((c[i][k]+c[k][j])*w[i][j]) + w_aux[i][j];
                if (l[i][j]>EPSILON){
                    phi_aux += l[i][j];
                }
            }
...
void Heuristicas::heuristica_construtiva(double hub_ativo, double phi){
    vector<int> arc_hub1;
    vector<int> arc_hub2;
    valor_solucao = phi;
    double phi_max;
    ...
    for(int k = 0; k < n; k++){ //Inicializa o conjunto de busca dos arcos entre concentradores
        for (int m = 0; m <  n; m++){
            if (m!=k){
    ...
    while (stop == 0){
        for(int i = 0; i < arc_hub1.size(); i++){
            h[arc_hub1[i]] = 1;
            h[arc_hub2[i]] = 1;
            z[km(n, arc_hub1[i], arc_hub2[i])] = 1;
            f_avaliacao = Shortest_path_algorithm();
            if (f_avaliacao > phi_aux){
                phi_aux = f_avaliacao;
                posicao = i;
            }
            h = h_copia;
        ...
        if (phi_aux > phi_max){
            phi_max = phi_aux; //atualiza o valor da função de avaliacao para o melhor valor encontrado
            arc_hub1.erase(arc_hub1.begin()+posicao); //retira o arco (k*, m*) do conjunto de busca
            arc_hub2.erase(arc_hub2.begin()+posicao); //retira o arco (k*, m*) do conjunto de busca
            h[arc_hub1[posicao]] = 1; //adiciona k* ao conjunto de hubs ativos
            h[arc_hub2[posicao]] = 1; //adiciona m* ao conjunto de hubs ativos
            z[km(n, arc_hub1[posicao], arc_hub2[posicao])] = 1; //adiciona o arco (k*,m*) ao conjunto de arcos ativos
            h_copia = h;
            z_copia = z;
        }else{
            stop =1;
    ...
    valor_solucao = phi_max;
    arc_hub1.clear();
    arc_hub2.clear();
}

void Heuristicas::heuristica_parcialmente_gulosa(double hub_ativo, double phi){
    vector<int> arc_hub1;
    vector<int> arc_hub2;
    valor_solucao = phi;
    double phi_max;
    ...
    while (stop == 0){
        for(int i = 0; i < 0.2*arc_hub1.size(); i++){
            posicao2 = rand () % arc_hub1.size();
            h[arc_hub1[posicao2]] = 1;
            h[arc_hub2[posicao2]] = 1;
            z[km(n, arc_hub1[posicao2], arc_hub2[posicao2])] = 1;
            f_avaliacao = Shortest_path_algorithm();
            if (f_avaliacao > phi_aux){
                phi_aux = f_avaliacao;
                posicao = posicao2;
...
double Heuristicas::Shortest_path_algorithm(){ /// (Elisangela)
  ...
  for(int k = 0; k < n; k++){ /// Monta uma lista de hubs
    if(h[k] >= (1 - EPSILON)){
      mp.insert(pair<double,vector<int> >(-s_chapeu[k], {0, k}));
    ...
    for(list<int>::iterator m = hub_list.begin(); m != hub_list.end(); m++){
      if(k != m){
        if( z[km(n, *k, *m)] >= (1 - EPSILON)){
          mp.insert(pair<double,vector<int> >(-g_chapeu[*k][*m], {1, *k, *m}));
  ...
  for(int i = 0; i < n; i++){
    for(int j = 0; j < n; j++){
      ...
      if( r[i][j] - sp_matrix[i][j] > EPSILON){
            mp.insert(pair<double,vector<int> >(-(r[i][j] - sp_matrix[i][j] )*w_chapeu[i][j], {2, i, j}));
  ...
  int l=0;
        ...
        while(mp.size()>0 && l < floor(Gamma)){

                it=mp.begin();

                if( (it->second)[0] == 0 ){ //verifica se é parametro de hub
                    sol_value += it->first;
                }else if( (it->second)[0] == 1 ){ //verifica se é parametro de arco hub
                    sol_value += it->first;
                }else{ // é parametro da demanda
                    sol_value += it->first;
                }
                mp.erase(it);
                l++;
         ...
         if(Gamma-floor(Gamma) > EPSILON){
            if(!mp.empty()){
                it=mp.begin();
                 if( (it->second)[0] == 0 ){ //verifica se é parametro de hub
                    sol_value += (Gamma-floor(Gamma))*it->first;
                }else if( (it->second)[0] == 1 ){ //verifica se é parametro de arco hub
                    sol_value += (Gamma-floor(Gamma))*it->first;
                }else{ // é parametro da demanda
                    sol_value += (Gamma-floor(Gamma))*it->first;
