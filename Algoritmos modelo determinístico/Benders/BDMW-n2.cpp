...
#define TimeLimit 172000 //36000 172000
        ...
        if(best_inc != inc_value){ /// Evita gerar cortes para um mesma solução
			...
			for(int i = 0; i < bd->n; i++){
			  for(int j = 0; j < bd->n; j++){
			    if(getValue(OptCut[ij(bd->n, i, j)]) >= 1){
...
int main (int argc, char *argv[]){
   ...
   //     bd->cplexsp.setParam(IloCplex::Threads,1);
       ...
       // while(bd->gap > EPSILON && bd->cplex.getTime() < TimeLimit){
          ...
          bd->GeraCortes(OptCut); /// Chama a função GeraCortes  para gerar corte com ponto MW e guarda o corte gerado na expressão OptCut
          for(int i = 0; i < bd->n; i++){
            for(int j = 0; j < bd->n; j++){
              bd->mod.add(OptCut[ij(bd->n, i, j)] <= 0.000000); /// Adiciona o corte de Benders relativo a expressão OptCut
              bd->NCuts++;
            }
          }
          OptCut.endElements();
			...
			/// =======================
			bd->cplex.solve();
		//	bd->nos += bd->cplex.getNnodes(); /// Recuperar nós com apenas Usercut ativa 01
	...
	// }
            ...
            if(bd->Activate_Lazy_Callback == 0){
			 	if(bd->cplex.getStatus() == IloAlgorithm::Optimal){
			 	  ...
			 	  bd->cplex.getValues(bd->vz,bd->z); /// Recupera a solução do mestre
					...
					bd->gap = (bd->ub  - bd->lb )/bd->lb ; /// Atualiza o gap
					u++;		      
					cout<<u<<"\t"<<bd->lb <<"\t"<<bd->ub <<"\t"<< (100*bd->gap)<<"\% \t"<<crono.getTime()<<"\t"<<bd->NCuts<<"\t"<<bd->cplex.getNnodes()<<"\t"<<endl;
				}
				else{
					break;
        ...
        // Print main output
        //===========================
        ofstream arq_saida("Resultados-n2-bdmw-shortestpath.txt",std::ofstream::app);
		if (!arq_saida) {cerr << "Erro arquivo \n"; exit(0);}
		
        arq_saida<<argv[1]<<"\t"<<bd->alpha<<"\t"<<bd->r[0][0]<<"\t"<<100 *  bd->gap<<"\t \t"<< fixed << setprecision(2)<<bd->lb<<"\t"<<bd->ub<<"\t"<<(double) crono.getTime()<<"\t"<<u<<"\t"<<bd->Activate_Lazy_Callback<<"\t"<<bd->Activate_User_Callback<<"\t"<<bd->NCuts<<"\t"<<bd->NNodes<<"\t"<<endl;
		    cout<<argv[1]<<"\t"<<bd->alpha<<"\t"<<bd->r[0][0]<<"\t"<<100 *  bd->gap<<"\t \t"<< fixed << setprecision(2)<<bd->lb<<"\t"<<bd->ub<<"\t"<<(double) crono.getTime()<<"\t"<<u<<"\t"<<bd->Activate_Lazy_Callback<<"\t"<<bd->Activate_User_Callback<<"\t"<<bd->NCuts<<"\t"<<bd->NNodes<<"\t"<<endl;
